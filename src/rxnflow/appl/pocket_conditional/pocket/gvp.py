...
class GVP_embedding(nn.Module):
    ...
    def forward(self, g: gd.Batch, aggr: str = "mean") -> tuple[torch.Tensor, torch.Tensor]:
        p_embed = self.run(
            (g.node_s, g.node_v),
            g.edge_index,
            (g.edge_s, g.edge_v),
        ...
        if aggr == "mean":
            z = scatter_mean(p_embed, g.batch, dim=0, dim_size=g.num_graphs)
        else:
            z = scatter_add(p_embed, g.batch, dim=0, dim_size=g.num_graphs)
        return p_embed, z

    def run(self, h_V, edge_index, h_E, seq):
        """
